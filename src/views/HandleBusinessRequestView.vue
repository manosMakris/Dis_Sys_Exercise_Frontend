...
const urlRef = computed(() => {
    return 'http://localhost:9090/api/businessRequests/getById/' + businessRequestIdRef.value;
});
...
function onAccept() {
    const urlRef2 = ref(`http://localhost:9090/api/businessRequests/acceptBusinessRequest/${data.value.id}`);
...
function onReject() {
    const urlRef3 = ref(`http://localhost:9090/api/businessRequests/rejectBusinessRequest/${data.value.id}`);
